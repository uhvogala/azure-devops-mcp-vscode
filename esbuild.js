...
.branches { display: grid; grid-template-columns: minmax(0, 1fr) auto; align-items: center; gap: 8px; margin-top: 12px; padding: 6px 10px; border-left: 3px solid var(--color-border-info, #1d4ed8); border-radius: 0 var(--border-radius-sm, 6px) var(--border-radius-sm, 6px) 0; background: var(--color-background-secondary, #f1f5f9); color: var(--color-text-secondary, #475569); font-size: 12px; } .branch-name { overflow-wrap: anywhere; } .branch-actions { display: flex; gap: 6px; } .branch-error { grid-column: 1 / -1; color: var(--color-text-danger, #b91c1c); font-size: 11px; overflow-wrap: anywhere; } .branch-error:empty { display: none; } button.checkout-branch { padding: 4px 8px; border-color: var(--color-border-primary, #64748b); background: transparent; color: var(--color-text-primary, #172b4d); font-size: 11px; }
...
.files { margin-top: 18px; border: 1px solid var(--color-border-primary, #e2e8f0); border-radius: var(--border-radius-sm, 6px); overflow: visible; }
.files-header { padding: 10px 12px; background: var(--color-background-secondary, #f8fafc); }
.file { padding: 10px 12px; border-top: 1px solid var(--color-border-primary, #e2e8f0); gap: 10px; } .file.reviewed { background: var(--color-background-secondary, #f8fafc); } .file-details { flex: 1 1 auto; min-width: 0; display: grid; align-content: center; gap: 3px; } code { overflow-wrap: anywhere; font-family: var(--font-mono, monospace); font-size: 13px; } button.comment-count { justify-self: start; padding: 0; border: 0; background: transparent; color: var(--color-text-info, #1686c3); font-size: 11px; font-weight: 600; text-decoration: underline; } button.comment-count:hover { background: transparent; color: var(--color-text-info, #1686c3); } .action-error { color: var(--color-text-danger, #b91c1c); font-size: 11px; line-height: 1.35; overflow-wrap: anywhere; } .file-actions { display: flex; flex: 0 0 auto; align-items: center; gap: 6px; } .review-control { display: inline-flex; flex: 0 0 92px; align-items: center; gap: 5px; color: var(--color-text-secondary, #64748b); font-size: 11px; cursor: pointer; } .review-control input { width: 14px; height: 14px; accent-color: #1686c3; cursor: pointer; } .review-control input:disabled { cursor: progress; }
...
<body><script>${script}</script></body>
	...
	await esbuild.build({
		entryPoints: ['src/pr-overview.js'],
		bundle: true,
		format: 'iife',
		platform: 'browser',
		target: 'es2022',
		outfile: 'dist/pr-overview.js',
		logLevel: 'silent',
	});
}
